...
calibration.instruction =
  "Please fixate the dot<br />that will appear. <br /><br />Do not move your head,<br />only use your eyes.";
...
design.blocks = [
  ...
  },
  {
    ...
    iti: 500,
    instruction:
      "Please look at the cross.<br />An image will follow. You are free <br />to look at the image.<br /><br />Do not move your head, <br />only use your eyes.",
    instruction_duration: 5000,
  },
  {
    type: "freeview",
...
function sendEmail() {
  $.ajax({
    ...
    .error(function (e) {
      $("#message")
        .html("ERROR: Message could not be sent.")
        .fadeIn(500)
        .delay(1000)
        .fadeOut(1000);
    });
...
function navigation(direction) {
  ...
  if (page == 2 && direction == "continue") {
    if ($("#consent_y").prop("checked")) {
      ...
      $("#message")
        .html("Please accept the consent form before continuing.")
        .fadeIn(500)
        .delay(1500)
        .fadeOut(500);
    }
    ...
    if (ss == "cf") {
      ...
      d_ident =
        $("#f_crowdflowerid").val() != ""
          ? "cf_" + $("#f_crowdflowerid").val()
          : "cf-";
    } else if (ss == "rub") {
      d_ident =
        $("#f_rubid").val() != "" ? "rub_" + $("#f_rubid").val() : "rub-";
    } else {
      d_ident = $("#f_asd").val() != "" ? "asd_" + $("#f_asd").val() : "asd-";
      if (
        d_ident == "asd_autism" ||
        d_ident == "asd_aspergers" ||
        d_ident == "asd_other"
      ) {
        design.blocks = [
          {
    ...
    if (d_consent !== undefined) {
      if (d_birthday != "" && d_gender != "" && d_visual != "") {
        ...
        $("#message")
          .html("Please fill out the form before continuing.")
          .fadeIn(500)
          .delay(1500)
          .fadeOut(500);
      }
  ...
  page = page < 1 ? 1 : page;
  page =
    page > $(".instruction_page").length ? $(".instruction_page").length : page;
