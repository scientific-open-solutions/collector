function correct_master() {
  ...
  master.surveys.user_surveys = Collector.missing_object(
  ...
  master.phasetypes.default = Collector.missing_object(
    master.phasetypes.default
  );
  master.phasetypes.user = Collector.missing_object(master.phasetypes.user);
  master.phasetypes.graphic = Collector.missing_object(
    master.phasetypes.graphic
  );
  master.phasetypes.graphic.files = Collector.missing_object(
    ...
    try {
      ...
      all_procs.forEach(function (this_proc) {
        if (typeof this_project.all_procs[this_proc] === "object") {
          this_project.all_procs[this_proc] = Papa.unparse(
            this_project.all_procs[this_proc]
          );
        }
        this_project.all_procs[this_proc] = this_project.all_procs[
          this_proc
        ].replace("trial type,", "code,");

        this_project.all_procs[this_proc] = Collector.PapaParsed(
          this_project.all_procs[this_proc]
        );
      });
  ...
  if (
    (typeof master.phasetypes.graphic.files === "undefined") &
...
function correct_user() {
  if (typeof user.data_folder === "undefined" || user.data_folder === "") {
    bootbox.confirm(
      "You don't (yet) have a folder where we'll put your data <b>when you test participants <u>on this device</u></b>. You're about to be asked where you would like this data to go. Please think carefully about this to make sure that your participant data is secure.",
      function (result) {
        if (result) {
...
Collector.custom_alert = function (msg, duration) {
  ...
  var top_padding =
    parseFloat($("#sim_navbar").css("height").replace("px", "")) +
  ...
  */
  ...
  this_alert.animate({ opacity: "1" }, 500, "swing");
...
Collector.start = function () {
  ...
  if (
    ...
    typeof user.current.path === "undefined"
  ) {
