  ...
  function until_successful_script(script_list, data, after_function) {
    if (script_list.length > 0) {
      ...
      function recursive_save(
        ...
        if (attempt_no == 10) {
          until_successful_script(script_list, data, after_function);
        } else {
          ...
          $.ajax({
            ...
            attempt_no++;
            console.dir("failed to save, attempting again");
            recursive_save(save_script_url, data, attempt_no, after_function);
          });
        }
      }
      recursive_save(save_script_url, data, 0, after_function);
    } else {
      /*
      ...
      if (data.trial_no == "_all_data") {
        precrypted_data(
          project_json,
          "We've tried to send your data, but it has failed multiple times. Please accept the download and send it directly to the researcher. What do you want to save this file as?"
        );
      }
    }
  }
