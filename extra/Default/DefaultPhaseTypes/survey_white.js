...
$("#proceed_button").on("click", function () {
  var current_table= $('table').filter(function() {return $(this).attr('style') === undefined || $(this).attr('style') === '';}).attr('id');
  var next_table_no = eval(current_table_no) + eval(1);
  var tableCount = $('table').length - 1; // this is reduced by 1 as the table numbering starts at 0

//   // QWERTY To get branching and page_breaks to work...
//   // (1) Check whether the 
//   console.log(current_table)
//   console.log(next_table_no )


//   function areAnyRowsWithBlockNameVisible() {
//     // Get all rows of the table
//     let rows = $("#" + current_table + " " + 'tr[block_name]');
//     // Iterate through the rows to check visibility
//     for (let row of rows) {
//         // Check if the row is visible
//         if (row.offsetParent !== null) {
//             return true; // At least one row with block_name is visible
//         }
//     }
//     next_table_no = eval(tableCount) + eval(1)
//     console.log(next_table_no)
//     console.log(eval(tableCount) + eval(1))
//     return false; // No rows with block_name are visible
   
// }

// areAnyRowsWithBlockNameVisible();
...
//row.values = row.values == "" ? row.answers : row.values; //qwerty1
...
function response_check(submitted_element) {
  if (submitted_element.type === "select-one" || submitted_element.type === "radio") {
    // hiding blocks, currently cannot handle two independent blocks but can handle a block embedded within another (not sure what I meant by this now CD)
