...
encrypt_obj = {
  _decrypt_request: function (
    ...
    switch (batch_status) {
      ...
      case "end":
        bootbox.prompt({
          title:
            "What is your password? (this will never be saved, but is needed)",
          inputType: "password",
            ...
            if (user_password) {
              ...
              function combine_password_key(keys_list, user_password) {
                if (keys_list.length === 0) {
                  ...
                  var decrypted_private_key_obj = CryptoJS.AES.decrypt(
                    your_stuff.keys.encrypted_private_key,
                    user_password
                  );
                  try {
                    var decrypted_private_key =
                      decrypted_private_key_obj.toString(CryptoJS.enc.Utf8);
                  } catch (error) {
                    Collector.custom_alert(
                      error + " trying with an archived private key"
                    );
                      ...
                      try {
                        ...
                        this_decrypted_message = JSON.parse(
                          encrypt_obj.decrypt(
                            ...
                            JSON.parse(encrypted_content)
                          )
                        );

                        this_decrypted_message = this_decrypted_message.map(
                          function (row) {
                            try {
                  ...
                  bootbox.alert(
                    '<div class="progress"><div class="progress-bar" role="progressbar" style="width: 0%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100" id="decryption_progress"></div></div>'
                  );
                  recursive_decryption(
              ...
              function process_decrypted(this_decrypted_message, batch_single) {
                ...
                if (filenames.length > 1) {
                  bootbox.dialog({
                    title:
                      "Would you like to have multiple files or one large file?",
                    message:
                      "There are multiple files - would you like the files to be merged or decrypted separately?",
                    buttons: {
                      merge: {
                        label: "Merge",
                        ...
                        callback: function () {
                          ...
                          bootbox.prompt({
                            ...
                            value:
                              default_filename.replace(".csv", "") + ".csv",
                            callback: function (result) {
                              if (result) {
                                var filename =
                                  result.toLowerCase().replace(".csv", "") +
                                  ".csv";
                                Collector.save_data(
                                  filename,
                                  Papa.unparse(this_decrypted_message, {
                      ...
                      separate: {
                        ...
                        className: "btn-info",
                        callback: function () {
                          function recursive_username_decryption(filenames) {
                              ...
                              bootbox.prompt({
                                ...
                                callback: function (result) {
                                  if (result) {
                                    ...
                                    Collector.save_data(
                                      filename,
                                      Papa.unparse(this_participant_message, {
                                        quotes: false, //or array of booleans
                                        quoteChar: '"',
                                        escapeChar: '"',
                                        delimiter: ",",
                  ...
                  if (this_decrypted_message.length === 0) {
                    ...
                    bootbox.prompt({
                      ...
                      callback: function (result) {
                        if (result) {
                          var filename =
                            result.toLowerCase().replace(".csv", "") + ".csv";
                          Collector.save_data(
                            filename,
                            Papa.unparse(this_decrypted_message, {
                              quotes: false, //or array of booleans
              ...
              if (typeof your_stuff.keys.archived === "undefined") {
                your_stuff.keys.archived = [];
              }
              var all_private_keys = [];
              your_stuff.keys.archived.forEach(function (row) {
                all_private_keys.push(row.encrypted_private_key);
              });
              all_private_keys.push(your_stuff.keys.encrypted_private_key);
  ...
  add_script: function () {
    bootbox.prompt(
      "Please complete the instructions on how to automatically have your data saved or e-mailed and then copy in the script address below",
      function (this_url) {
        /*
  ...
  archive_keys: function () {
    var keypair = {
      public_key: your_stuff.keys.public_key,
      encrypted_private_key: your_stuff.keys.encrypted_private_key,
    };
    your_stuff.keys.archived.push(keypair);
  },
  change_password: function () {
    bootbox.dialog({
      title: "Are you sure you want to update your password?",
      message:
        "We will still try to decrypt data that was encrypted with your old password, but make sure to use your new password to decrypt new data.",
      buttons: {
        yes: {
          label: "Okay",
          className: "btn-primary",
  ...
  confirm_keys: function () {
    ...
    bootbox.prompt(
      "Let's quadruple check everything is working - if you put in your password you should see the message 'howdy', which has just been encrypted using your public key",
      function (user_password) {
        var decrypted_private_key_obj = CryptoJS.AES.decrypt(
          your_stuff.keys.encrypted_private_key,
          user_password
        );
        var decrypted_private_key = decrypted_private_key_obj.toString(
          CryptoJS.enc.Utf8
        );
        this_decrypted_message = encrypt_obj.decrypt(
          decrypted_private_key,
          this_encrypted_message
        );
  ...
  decrypt: function (receiverSecretKey, encryptedData) {
    const receiverSecretKeyUint8Array =
      nacl.util.decodeBase64(receiverSecretKey);
  ...
  encrypt: function (receiverPublicKey, msgParams) {
    //: string
  ...
  generate_keys: function () {
    ...
    if (typeof your_stuff.keys === "undefined") {
      your_stuff.keys = {
        archived: [],
      };
    }
            ...
            function (result_2) {
              if (result === result_2) {
                ...
                if (plaintext === receiverSecretKey) {
                  your_stuff.keys.encrypted_private_key = encryptedAES_string;
                  encrypt_obj.confirm_keys();
                  list_keys();
                } else {
                  bootbox.alert(
                    "Something went wrong with creating your encryption keys"
                  );
                }
              } else {
                bootbox.alert(
                  "Erm, your passwords didnt match, do you want to have another go?"
                );
...
function list_data_servers() {
  var select_server_html =
    "<select class='form-control' id='select_data_server'>" +
    "<option disabled selected>--Please select a server--</option>";
  Object.keys(your_stuff.data.servers).forEach(function (this_server) {
    var this_server_info = your_stuff.data.servers[this_server];
...
function list_keys() {
  if (
    typeof your_stuff.keys !== "undefined" &&
    typeof your_stuff.keys.public_key !== "undefined" &&
    your_stuff.keys.public_key !== ""
  ) {
    $("#public_key").val(your_stuff.keys.public_key);
    $("#private_key").val(your_stuff.keys.encrypted_private_key);
  } else {
    encrypt_obj.generate_keys();
  }
}
...
function loadFileAsText() {
  ...
  function batch_no_batch(batch_no) {
    ...
    var default_filename = document
      .getElementById("fileToLoad")
      .files[batch_no].name.toLowerCase()
      .replace(".txt", "");
    var fileReader = new FileReader();
    fileReader.onload = function (fileLoadedEvent) {
      var textFromFileLoaded = fileLoadedEvent.target.result;
      encrypt_obj._decrypt_request(
        textFromFileLoaded, //file_content
        default_filename,
        batch_status
      );
    };
...
function populate_data_div(data_div_id, data_obj, bootstrap_class) {
  ...
  var this_list = $("<div>")
    ...
    .append(
      $("<div>")
        ...
        .append($("<div>").prop("id", "project_column").addClass("tab-content"))
    );
  ...
  Object.keys(projects).forEach(function (project_id) {
    ...
    projects[project_id].forEach(function (this_pp) {
      ...
      $("#" + "participant-list-" + project_id).append(
        $("<div>")
          .append(
            $("<button>")
              .addClass("btn")
              ...
              .html(
                "Download<span style='display:none'>" +
                  project_id +
                  "_____" +
          ...
          .append($("<span>").html(this_pp.hashed_user_id))
...
function request_data_list() {
  ...
  $.post(
    your_stuff.data.servers[$("#select_data_server").val()].registration_url,
    {
    ...
    function (result) {
      ...
      var server_data_size =
        mega_data_obj.server_data_obj.reduce(function (a, b) {
      ...
      */
      ...
      $(".delete_server_data_btn").on("click", function () {
        ...
        $.post(
          your_stuff.data.servers[data_server].registration_url,
          {
            email: $("#data_user_email").val(),
            password: $("#data_user_password").val(),
          ...
          function (result) {
            ...
            mega_data_obj.server_data_obj =
              mega_data_obj.server_data_obj.filter(
      ...
      $(".download_storage_data_btn").on("click",function(){
        var this_folder = $(this).find("span")[0].innerHTML;
        $.post(your_stuff.data.servers[$("#select_data_server").val()].registration_url,{
          email:       $("#data_user_email").val(),
          password:    $("#data_user_password").val(),
