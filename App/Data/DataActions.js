...
$("#google_script_btn").on("click", function () {
  $.get("Data/Google/CollectorSave.txt", function (google_script) {
    bootbox.alert(
      "//Select and copy the following to a <b>google apps script</b>: <br><br><textarea style='white-space: pre-wrap;' onclick='this.focus();this.select()' readonly='readonly'>" +
        google_script +
        "</textarea>"
    );
  });
...
$("#list_data_btn").on("click", function () {
  if ($("#data_user_email").val() == "") {
    bootbox.alert(
      "Please put in your email address that is registered with the server selected."
    );
