...
function initiate_actions() {
  ...
  function valid_new_name(this_name) {
    ...
    var current_code = Object.keys(your_stuff.phasetypes.user).concat(
  ...
  $("#ACE_editor").on("keyup input", function () {
    ...
    var code_file = your_stuff.phasetypes.file;
    if (typeof your_stuff.phasetypes.user[code_file] === "undefined") {
      your_stuff.phasetypes.user[code_file] = {
        files: {},
      };
    }
    your_stuff.phasetypes.user[code_file].updated = true;
    your_stuff.phasetypes.user[code_file] = ace_content;
  });
          ...
          if (firstResponse) {
            bootbox.confirm({
              ...
              callback: function (result) {  
                parent.parent.functionIsRunning = false;
                if (result) {
                  delete your_stuff.phasetypes.graphic.files[deleted_phasetype];
            ...
            callback: function () {
              ...
              if (protected_name_check(new_name)) {
                if (valid_new_name(new_name)) {
                  ...
                  your_stuff.phasetypes.user[new_name] = content;
                  your_stuff.phasetypes.file = new_name;
            ...
            callback: function () {
              ...
              if (protected_name_check(new_name)) {
                if (valid_new_name(new_name)) {
                  ...
                  your_stuff.phasetypes.graphic.files[new_name] = { elements: {}, };
                  your_stuff.phasetypes.graphic.files[new_name].width = "1920px";
                  your_stuff.phasetypes.graphic.files[new_name].height = "1080px";
                  your_stuff.phasetypes.graphic.files[new_name].aspect = "lg";
                  your_stuff.phasetypes.graphic.files[new_name]["background-color"] = "#ffffff";
                  your_stuff.phasetypes.graphic.files[new_name].mouse_visible = true;
                  your_stuff.phasetypes.graphic.files[new_name].keyboard = { valid_keys: "", end_press: true, };
                  your_stuff.phasetypes.file = new_name;
                  // graphic_editor_obj.update_main_settings();
                  // graphic_editor_obj.clean_canvas();
                  graphic_editor_obj.load_canvas(your_stuff.phasetypes.graphic.files[new_name].elements);
  ...
  $("#rename_phasetypes_button").on("click", function () {
    if (!parent.parent.functionIsRunning) {
      ...
      if (typeof your_stuff.phasetypes.default[phasetype_selected] !== "undefined") {
        ...
        bootbox.prompt(
          ...
          function (new_name) {
            if (new_name === null) {
              ...
              your_stuff.phasetypes.user[new_name] = your_stuff.phasetypes.user[original_name];
              delete your_stuff.phasetypes.user[original_name];

              var response = CElectron.fs.write_file("PhaseTypes/",new_name.replace(".html", "") + ".html",your_stuff.phasetypes.user[new_name]);
              
              if (response === "success") {
                if (your_stuff.phasetypes.graphic.files[original_name]){
                  console.log(original_name)
                  var graphic_objcontent = JSON.stringify(your_stuff.phasetypes.graphic.files[original_name]);
                ...
                // Lastly, we just do a your_stuff "save" to ensure the change is kept after quitting Collector
                setTimeout(function() { 
                  $("#save_phasetype_btn").click();
                  $("#save_btn").click();
  ...
  $("#save_phasetype_btn").on("click", function () {
    if ($("#phasetype_select").val() !== null) {
      ...
      if (typeof your_stuff.phasetypes.default[name] === "undefined") {
  ...
  $("#phasetype_select").on("change", function () {
    ...
    your_stuff.phasetypes.graphic.files = {};
    var code_file = your_stuff.phasetypes.file;
    ...
    if (old_code !== "" && (Object.keys(your_stuff.phasetypes.default).indexOf(old_code) === -1)) {
      code_obj.save(your_stuff.phasetypes.user[old_code], old_code, "old");
    }
    ...
    your_stuff.phasetypes.file = code_file;

    if (your_stuff.phasetypes.default[code_file] == null) {
      user_default = "user";
    ...
    // setTimeout(() => {
      ...
      your_stuff.phasetypes.graphic.files[code_file] = phasetype_object;
      if (phasetype_object != null) {      
        ...
        graphic_editor_obj.load_canvas(your_stuff.phasetypes.graphic.files[code_file].elements);
          // graphic_editor_obj.clean_canvas();
