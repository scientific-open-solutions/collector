      ...
      if (repo) {
        progress_bootbox({
          ...
          actions: [
            // "Checking you have an authentication token"
            function () {
              var this_response = CElectron.git.token_exists();
            ...
            // "Checking if the organization exists and you are a member of it"
            function () {
              var this_response = CElectron.git.valid_org({
                org: org,
              });
              if (this_response !== "success") {
                bootbox.alert(this_response);
            ...
            function () {
              ...
              var this_response = CElectron.git.add_repo({
                org: org,
                repo: repo,
              });
            ...
            // "Synching with online repository"
            function () {
              var this_response = CElectron.git.push({
                org: org,
                repo: repo,
            ...
            // "Activate the online repository as a website"
            function () {
              var this_response = CElectron.git.pages({
                org: org,
                repo: repo,
              });
              if (this_response !== "success") {
...
$("#add_token_btn").on("click", function () {
  bootbox.prompt(
    "Please copy and paste an authentication token you can generate by going to <br> <input readonly value='https://github.com/settings/tokens'/ onclick='this.select()' style='width:100%'> <br> On this page, select <b>Generate a new token</b>, and then select <b>write:packages</b> to get the correct privileges. You can change the <b>Expiration</b> to later (e.g. 90 days) to reduce how frequently you need to generate these tokens. Once you've made your selections, scroll to the bottom of the page to click on the <b>Generate token</b> button. You should then be able to copy the token into the text input below:",
    function (auth_token) {
      if (auth_token) {
        var this_response = CElectron.git.add_token(auth_token);
        if (this_response !== "success") {
          bootbox.alert(this_response);
        }
      }
...
$("#local_repo_btn").on("click", function () {
  var path = CElectron.find_path()[0];
  ...
  if(typeof(path) !== "undefined"){
    ...
    var repo_info = JSON.parse(CElectron.git.repo_info(path));
    ...
    function (confirmed) {
      if (confirmed) {
        ...
        progress_bootbox({
          ...
          actions: [
            function () {
              var pull_response = CElectron.git.pull({
                org: org,
                repo: repo,
              });
