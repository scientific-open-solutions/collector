...
*/
Collector.tests = {
  categories: ["code", "data", "helper", "mods", "projects", "surveys"],
  pipeline: ["start", "after_start"],
  pipe_position: "start",
  ...
  activate_pipeline: function () {
    ...
    if (old_index < Collector.tests.pipeline.length - 1) {
      ...
      this.categories.forEach(function (test_category) {
        Object.keys(Collector.tests[test_category]).forEach(function (
          this_test
        ) {
          if (
            Collector.tests[test_category][this_test].type ==
            Collector.tests.pipe_position
          ) {
  ...
  count_remaining_tests: function (output_span) {
    ...
    this.categories.forEach(function (test_category) {
      Object.keys(Collector.tests[test_category]).forEach(function (this_test) {
        if (
          Collector.tests[test_category][this_test].type == this_type &&
    ...
    if (
      tests_remaining == 0 &&
      Collector.tests.pipeline.indexOf(this_type) <
        Collector.tests.pipeline.length - 1
    ) {
      this.activate_pipeline(this_type);
    }
    $("#start_tests_remaining_span").html(tests_remaining);
  },
  fail: function (test_category, this_test, error) {
    Collector.tests[test_category][this_test].outcome = "fail";
    $("#test_" + test_category + "_" + this_test).html(
      "<span class='text-danger'>Fail</span>"
    );
    bootbox.alert("error occurred " + test_category + "-" + this_test);
  },
  pass: function (test_category, this_test) {
      ...
      this.categories.forEach(function (test_category) {
        ...
        Object.keys(Collector.tests[test_category]).forEach(function (
          ...
          test_text +=
            ...
            Collector.tests[test_category][this_test].text +
            "</td>" +
            "<td class='text-dark' " +
            "id='test_" +
            test_category +
